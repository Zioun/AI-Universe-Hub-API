...
const card = (data, seeMore) => {
    ...
    if (!seeMore) {
        ...
        const showBtnHide = document.getElementById("show-all-button");
        showBtnHide.classList.add("hidden");
    }

    cardArea.innerHTML = ''; // Clear existing data
    data.forEach((tool) => {
        ...
        <div class="card w-96 bg-base-100 shadow-xl border m-auto">
            ...
            <div class="card-body items-left text-left py-4 px-4">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div>
                        <button onclick="showDetails('${tool.id}')" class="bg-[#FEF7F7] text-[#EB5757] p-3 rounded-[50%]">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
// see more
const seeMore = () => {
    apiGet(false); // Initially show only 6 data
}

const showAllData = () => {
    apiGet(true); // Show all data
}
